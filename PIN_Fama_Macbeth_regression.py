...
def cross_section_regression(year):
    ...
    for month in range(7, 13):
        ...
        for code in all_codes:
            ...
            pin_row = PIN_Data[(PIN_Data['year'] == year) & (PIN_Data['code'] == code)]
            ...
            if spm_row.empty or pin_row.empty or market_row.empty or preranking_beta_row.empty:
                continue
            
            monthly_return = spm_row.iloc[0].get(f'r{month}', np.nan)
            pin = pin_row.iloc[0].get('PIN', np.nan)
            ...
            if any(pd.isna(v) for v in [monthly_return, pin, rf, LnMV, BPR, portfolio_num]) or monthly_return == STOCK_MISSING_VALUE or rf == MARKET_MISSING_VALUE or BPR <= 0:
                continue
            
            ingroup_beta = portfolio_beta[portfolio_beta['index'] == portfolio_num].iloc[0].get('beta')
            X.append([ingroup_beta, pin, LnMV, round(math.log(BPR), 5)])
...
def cross_section_regression_next_year(year):
    ...
    for month in range(1, 7):
        ...
        for code in all_codes:
            ...
            preranking_beta_row = preranking_beta[(preranking_beta['year'] == year) & (preranking_beta['code'] == code)]
            if spm_row.empty or spm_row_next_year.empty or pin_row.empty or market_row.empty or preranking_beta_row.empty:
                continue
...
output_file = 'PIN_Fama_Macbeth_coef.txt'
