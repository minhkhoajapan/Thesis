...
def calculate_pin(a, u, eb, es):
    return a * u / (a * u + eb + es)
...
def process_stock(stock_code):
    ...
    months = np.array([int(str(d)[:4]) for d in stock_data['date']])
    ...
    for year in year_range:
        ...
        if len(yearly_data) == 0:
            print(f"No trading data for Stock {stock_code} in financial year {year}")
            continue
        ...
        try:
            results = eo_model.fit(buys, sells)
            final_pin = calculate_pin(results['a'], results['u'], results['eb'], results['es'])
            formatted_values = " ".join(f"{value:.5f}" for value in results.values())
            results_list.append(f"{stock_code} {year} {formatted_values} {final_pin:.5f}")
            print(f"Processed successfully {stock_code} in financial year {year}")
        except Exception as e:
            print(f"Error processing stock {stock_code} in financial year {year}")
